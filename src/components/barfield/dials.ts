...
export type DialGroupName =
  | 'depth' | 'focus' | 'motion' | 'density' | 'bloom' | 'shape' | 'color / hud' | 'sound';
...
export interface RangeSpec {
  ...
  max: number;
  /**
   * Kept as the authored string as well as a number: the panel printed a rounded integer
   * for steps of 1 and 2 and two decimals otherwise, and the same string decides how many
   * decimals a snapped value keeps. Deriving both from one source keeps the readouts
   * identical to the panel they replace.
   */
  step: number;
  stepText: string;
  value: number;
...
export type DialSpec = RangeSpec | ToggleSpec;
...
const range = (
  id: NumericDialId, label: string, min: number, max: number, stepText: string, value: number,
): RangeSpec => ({ kind: 'range', id, label, min, max, step: Number(stepText), stepText, value });

const toggle = (id: BooleanDialId, label: string, value: boolean): ToggleSpec =>
  ({ kind: 'toggle', id, label, value });
...
export type NumericDialId =
  | 'zSpread' | 'parallax' | 'driftSpeed' | 'depthFade' | 'idleLife'
  | 'focusScale' | 'spaceGap' | 'spaceScale' | 'spaceDepth' | 'spaceFloat' | 'spaceWave'
  | 'shimmer' | 'flow' | 'restCalm' | 'snap'
  | 'cols' | 'barWidth' | 'gapNoise'
  | 'bloomThreshold' | 'bloomRadius' | 'bloomAmount' | 'exposure'
  | 'logoScale' | 'threshold' | 'edgeJitter' | 'blobWarp' | 'warpSpeed'
  | 'hue' | 'sat' | 'coreGlow'
  | 'volume' | 'tempo' | 'grit' | 'punch';

export type BooleanDialId = 'hudOn' | 'animate';

export type DialId = NumericDialId | BooleanDialId;

export type Dials = Record<NumericDialId, number> & Record<BooleanDialId, boolean>;

export const SPECS: Record<DialId, DialSpec> = Object.fromEntries(
  DIAL_GROUPS.flatMap((g) => g.dials.map((d) => [d.id, d])),
) as Record<DialId, DialSpec>;

/** The one live copy every module reads. Mutated in place so existing reads of `P.x` hold. */
export const P: Dials = Object.fromEntries(
  Object.values(SPECS).map((d) => [d.id, d.value]),
) as Dials;

type Listener = (id: DialId) => void;
const listeners = new Set<Listener>();

/** Replaces the sketch's single `onDialChange` hook. Returns an unsubscribe. */
export function onDialChange(fn: Listener): () => void {
  listeners.add(fn);
  return () => listeners.delete(fn);
}

function notify(id: DialId) {
  for (const fn of listeners) fn(id);
}

/** How many decimals the authored step keeps, so snapping can't drift into 0.5300000000001. */
function decimals(stepText: string): number {
 ...
 */
export function quantise(spec: RangeSpec, v: number): number {
...
export function setDial(id: NumericDialId, v: number): number;
export function setDial(id: BooleanDialId, v: boolean): boolean;
export function setDial(id: DialId, v: number | boolean): number | boolean {
  const spec = SPECS[id];
  const next = spec.kind === 'toggle' ? Boolean(v) : quantise(spec, Number(v));
  if (P[id] === next) return next;
  (P as Record<DialId, number | boolean>)[id] = next;
  notify(id);
  return next;
}
...
export function resetDials(): void {
  for (const spec of Object.values(SPECS)) {
    const set = setDial as (id: DialId, v: number | boolean) => void;
    set(spec.id, spec.value);
  }
}

/** The readout string for a dial, matching the panel's step-based precision. */
export function format(id: DialId): string {
  const spec = SPECS[id];
  if (spec.kind === 'toggle') return P[spec.id] ? 'on' : 'off';
  const n = P[spec.id];
 ...
 */
export function dialText(): string {
