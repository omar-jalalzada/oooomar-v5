 ...
 */
import { clock, effect, frameLoop, init, sampler, surface, target } from 'vgpu';
import { BLUR_WGSL, BRIGHT_WGSL, COMPOSITE_WGSL, FIELD_WGSL } from './shaders';
import { P, onDialChange } from './dials';
import { drawHud, drawTagline, initHud } from './hud';
import { updateLetters } from './letters';
import {
  HOT, M, RAVE_HOLD, RAVE_MAX, approach, autoGain, clamp, els, pointer, rave,
  sound, states, vis, viewport,
} from './state';
...
export interface FieldRefs {
  ...
  tagline: HTMLElement;
  /** The sound control, placed under the tagline by the frame loop. */
  pill?: HTMLElement | null;
  /** Dev-only: the dial panel, the mode tell and the fps readout. Absent in production. */
...
function measureViewport(panel: HTMLElement | null | undefined) {
 ...
 */
export async function startField(refs: FieldRefs): Promise<boolean> {
  ...
  const small = ([w, h]: readonly [number, number]): [number, number] =>
    [Math.max(1, Math.floor(w / shrink)), Math.max(1, Math.floor(h / shrink))];
  ...
  onDialChange((id) => {
    ...
    if (id === 'bloomThreshold' || id === 'bloomAmount' || id === 'exposure') raveApplied = -1;
    ...
    if (id === 'grit') sound.engine?.setGrit(P.grit);
    if (id === 'tempo') sound.engine?.setTempo(P.tempo);
  });

  view.onResize(({ width, height }: { width: number; height: number }) => {
  ...
  gpu.onError((err: { code?: string; message?: string }) =>
    console.error('[vgpu]', err.code ?? '', err.message ?? err));
  ...
  frameLoop(gpu, (frame) => {
    ...
    if (animate) M.mtime += dt * ((P.tempo || 115) / 60);
    ...
    const env = (cur: number, tgt: number) => approach(cur, tgt, 0.045, dt);
    ...
    if (refs.mode) {
      ...
      if (refs.mode.textContent !== modeNow) refs.mode.textContent = modeNow;
