...
import { LETTERS, M, TAU, VB_CY, approach, clamp, els, grab, pointer, states, viewport, } from './state.js';
import { hitTest } from './input.js';
export function updateLetters(t, dt) {
    ...
    M.focusIdx = -1;
    let focus = 0;
    states.forEach((st, i) => { if (st.hover > focus) {
        focus = st.hover;
        M.focusIdx = i;
    } });
    ...
    if (stacked) {
        ...
        baseScale = Math.min((w * 0.78) / (widestU * s), avail / (glyphU * s * (4 + 3 * GAP)));
