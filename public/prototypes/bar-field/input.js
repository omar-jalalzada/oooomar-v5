 ...
 */
import { P, SPECS, setDial } from './dials.js';
import { LETTER_DIAL, els, grab, pointer, states } from './state.js';
/**
...
export function bindInput() {
    window.addEventListener('pointerdown', (e) => {
        if (e.button !== 0 || onChrome(e.target))
            return;
    ...
    els.stage?.addEventListener('touchstart', (e) => {
        const t = e.touches[0];
        if (t && hitTest(t.clientX, t.clientY) >= 0)
            e.preventDefault();
    }, { passive: false });
}
