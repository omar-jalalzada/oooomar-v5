...
import { RAVE_KICK, rave } from './state.js';
...
export function createAudio() {
    ...
    const setGrit = (amount) => {
        if (Math.abs(amount - gritAt) < 0.005)
            return;
        gritAt = amount;
    ...
    function scheduleStep(i, t, s16) {
        ...
        if (st === 0 && rl > 0.5 && Math.floor(i / 16) % 2 === 1)
            riser(t, s16 * 15);
        const kl = lv('kick');
        if (kl > 0.04)
            kick(t, kl * pu);
        ...
        const hl = lv('hat');
        if (hl > 0.04)
            hat(t, hl * 0.13 * pu, op > 0.3);
        else if (op > 0.04)
            hat(t, op * 0.14 * pu, true);
        const gh = lv('hatGhost');
        if (gh > 0.04)
            hat(t, gh * 0.045 * pu, false);
        const sh = lv('shake');
        if (sh > 0.04)
            shaker(t, sh * 0.075 * pu);
        const rd = lv('ride');
        if (rd > 0.04)
            ride(t, rd * 0.05 * pu);
        ...
        beds.forEach(([gr, wt]) => {
            ...
            if (ti >= 0)
                tom(t, gr.chord.root - 12 + TOM_WALK[ti % 6], wt * 0.5 * pu);
            gr.sub.forEach(([sst, semis, len]) => {
                if (sst === st)
                    sub(t, B.chord.root - 24 + semis, s16 * len, bed);
            });
            if (has(gr.stab, st))
                stab(t, B.chord, s16 * gr.stabLen, bed);
        });
    ...
    return {
        ...
        musicState() {
            ...
            const [wA, wB] = weights(blend);
            const weight = { kick: 1, clap: 0.7, tom: 0.6, open: 0.5,
                stab: 0.45, hat: 0.4, ride: 0.3, shake: 0.25, hatGhost: 0.2 };
            const keys = Object.keys(weight);
            const density = [];
            for (let k = 0; k < 16; k += 1) {
                let d = 0;
                const live = [[GROOVES[prevIdx], wA], [GROOVES[curIdx], wB]];
                live.forEach(([gr, wt]) => {
                    keys.forEach((key) => {
                        if (has(gr[key], k))
                            d += weight[key] * wt;
                    });
                    gr.sub.forEach(([sst]) => { if (sst === k)
                        d += 0.4 * wt; });
                });
